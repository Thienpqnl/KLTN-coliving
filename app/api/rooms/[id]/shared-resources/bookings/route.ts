...
export async function GET(
  ...
  try {
    ...
    const proxied = await tryProxyCommunityService({
      identity: user,
      path: `/v1/rooms/${roomId}/shared-resources/bookings`,
      fallbackMessage: "Không thể tải lịch tài nguyên",
    });
...
export async function POST(
  ...
  try {
    const user = await getAuthUser(request);
    const { id: roomId } = await params;
    const validated = createBookingSchema.parse(await request.json());
    const proxied = await tryProxyCommunityService({
      identity: user,
      path: `/v1/rooms/${roomId}/shared-resources/bookings`,
      method: "POST",
      body: validated,
      successStatus: 201,
      fallbackMessage: "Không thể đặt lịch tài nguyên",
    });
    if (proxied) return proxied;
    ...
    const booking = await prisma.$transaction(async (tx) => {
      ...
      `;
      if (lockedResourceRows.length === 0) throw new ApiError(404, "Tài nguyên không tồn tại");
      const resource = await tx.sharedResource.findUnique({ where: { id: validated.resourceId } });
      if (!resource) throw new ApiError(404, "Tài nguyên không tồn tại");
      if (resource.status === "MAINTENANCE") throw new ApiError(409, "Tài nguyên hiện đang bảo trì, không thể đặt lịch");
      ...
      if (validated.startTime < new Date()) throw new ApiError(400, "Không thể đặt lịch trong quá khứ");
      const existingBookings = await tx.resourceBooking.findMany({
        where: { resourceId: validated.resourceId, status: { in: ["PENDING", "APPROVED"] } },
