...
// Parse area format like "20 m2" to string (keeping original format for flexibility)
function parseArea(areaStr: string): string {
  return areaStr.trim();
}
...
function readCSV(filePath: string): CSVRoom[] {
  ...
  const rooms: CSVRoom[] = dataLines.map(line => {
  ...
  try {
    ...
    for (const room of rooms) {
      ...
      try {
        ...
        imported++;
        console.log(`✅ [${imported}] ${room.title.substring(0, 50)}...`);
      } catch (error) {
        console.error(`❌ Failed to import: ${room.title}`);
        console.error(error);
        skipped++;
      }
    }
